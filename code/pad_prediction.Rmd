...
ggplot(per_call_df, aes(call_time))+
  geom_histogram(aes(fill=as.factor(content)), position = "identity", bins = 30, alpha = 0.7)+
  scale_fill_manual(
    name = "Content\nReached",
    values = c("red", "darkgreen"),
    labels = c("No", "Yes"))+
  labs(
    title='Distribution of Calls by Time',
...
t.test(no_made$call_time, made_it$call_time, alternative = "two.sided", var.equal = FALSE)

lm_nocontrol <- lm(content ~ call_time, per_call_df)
summary(lm_nocontrol)
```
...
#t.test(time_to_content$to_content, no_made$call_time, alternative = "two.sided", var.equal = FALSE)
